package edu.upc.sad.chat.server;

import edu.upc.sad.chat.utils.MyServerSocket;
import edu.upc.sad.chat.utils.MySocket;

import java.io.*;
import java.net.*;
import java.util.*;

public class Server{
	
	private static MyServerSocket serverSocket;
	public static volatile Map<String, MySocket> map;
	//public ReentrantLock lock;

	public static void main(String[] args) {
		try {
			map = new HashMap<String, MySocket>();
			serverSocket = new MyServerSocket(2500);
			System.out.println("Server started");
			while(true) {
				MySocket tmp = serverSocket.accept();
				System.out.println("Connection Accepted");
				new Thread(new Worker(tmp)).start();
			}
		} catch (Exception e) {
			
		}
	}

	private static class Worker implements Runnable{

		private MySocket sock;
		private String key;

		public Worker (MySocket sock) {
			this.sock = sock;
		}
	
		public void run() {
			try {
				key = sock.receive();
				System.out.println("Worker started: "+key);
				for(String keyTmp: map.keySet()) {
					(map.get(keyTmp)).send("UP:"+key);
					sock.send("WAS:"+keyTmp);
				}
				map.put(key, sock);
				String tmp;
				while((tmp = sock.receive())!= null) {
					System.out.println(tmp);
					for(String keyTmp: map.keySet()) {
						if (keyTmp != key) (map.get(keyTmp)).send(key+": "+tmp);
					}
				}
				//sock.close();
				for(String keyTmp: map.keySet()) {
					if (keyTmp != key) (map.get(keyTmp)).send("DOWN:"+key);
				}
				System.out.println("Worker dead");
				map.remove(key);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	}

}


